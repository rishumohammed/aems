    ...
    <div class="header-section pa-8 pb-15 mb-n10">
      <div class="d-flex align-center justify-space-between mb-2">
        <div class="d-flex align-center">
          ...
          <div>
            <h1 class="text-h4 font-weight-bold mb-1">Question Management</h1>
      ...
      <v-row v-if="!loading && question">
        ...
        <v-col cols="12" md="8">
              ...
              <div class="d-flex align-start">
                ...
                <div class="flex-grow-1">
                  <div class="d-flex justify-space-between align-center mb-1">
                    ...
                    <div class="d-flex align-center">
                      <div class="text-caption text-grey mr-2">{{ new Date(reply.created_at).toLocaleString() }}</div>
                      <v-btn icon="mdi-delete" size="x-small" color="error" variant="text" @click="deleteReply(reply.id)"></v-btn>
                    </div>
                  </div>
          ...
          <v-card flat rounded="xl" class="border shadow-soft pa-6">
            <h3 class="text-subtitle-1 font-weight-bold mb-4">Post an Admin Reply</h3>
            <v-textarea
              v-model="newReply"
        ...
        <v-col cols="12" md="4">
          <v-card flat rounded="xl" class="border shadow-soft pa-6 sticky-sidebar">
            <h3 class="text-h6 font-weight-bold mb-6">Moderation</h3>
            
            <div class="mb-6">
              <div class="text-subtitle-2 text-grey mb-2">Update Status</div>
              <v-select
                v-model="currentStatus"
                :items="['open', 'answered', 'resolved', 'pending_review', 'closed']"
                variant="outlined"
                density="comfortable"
                hide-details
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error('Error fetching admin QA detail:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitReply = async () => {
  ...
  try {
    ...
    if (question.value.status === 'open' || question.value.status === 'pending_review') {
...
const deleteQuestion = async () => {
  if (!confirm('Are you sure you want to completely delete this question and all replies?')) return;
  try {
    await api.delete(`/lms/qa/qa/${question.value.id}`);
    router.push('/dashboard/admin/qa');
  } catch (error) {
...
const deleteReply = async (id) => {
  ...
  try {
    await api.delete(`/lms/qa/qa/replies/${id}`);
    await fetchData();
  } catch(error) {
