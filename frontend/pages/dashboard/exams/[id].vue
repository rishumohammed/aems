<template>
  <v-container fluid class="pa-6">
    <div class="d-flex align-center gap-3 mb-8">
      ...
      <div>
        <h1 class="text-h4 font-weight-bold mb-1">Edit Exam</h1>
...
const save = async () => {
  ...
  try {
    ...
    // 1. Update Exam Settings
    await api.put(`/exams/${examId}`, {
      ...form.value,
      questions: undefined // Don't send questions to the main exam update route
    });

    // 2. Handle Question Deletions
    // Fetch current questions from DB to see which ones were removed in the UI
    const { data: currentQuestions } = await api.get(`/exams/${examId}`);
    const currentIds = currentQuestions.questions?.map((q: any) => q.id) || [];
    const newIds = form.value.questions.filter(q => q.id).map(q => q.id);
...
onMounted(async () => {
  try {
    ...
    courses.value = courseRes.data || courseRes;
    ...
    if (examData) {
      ...
      if (examData.proctoring_config) {
        try {
          parsedConfig = typeof examData.proctoring_config === 'string'
            ? JSON.parse(examData.proctoring_config)
            : examData.proctoring_config;
        } catch (e) {
      ...
      form.value = {
        ...
        proctoring_enabled: !!examData.proctoring_enabled,
        proctoring_config: parsedConfig,
        requires_scheduling: !!examData.requires_scheduling,
        enable_certificate: examData.enable_certificate !== undefined ? !!examData.enable_certificate : true,
        show_results: examData.show_results !== undefined ? !!examData.show_results : true,
        show_result_detail: !!examData.show_result_detail,
